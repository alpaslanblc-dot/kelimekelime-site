...
// Menü, Logo ve Modal Enjeksiyonu (Aynen kalsın, sadece submit fonksiyonunu düzeltiyoruz)
document.body.insertAdjacentHTML('beforeend', `
    <div class="overlay" id="overlay" onclick="toggleMenu()"></div>
    <div class="side-menu" id="sideMenu">
        <div class="menu-section"><h4>Kategoriler</h4><div id="menuCats"></div></div>
    ...
    <div id="customModal" style="display:none; position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.6); z-index:3000; align-items:center; justify-content:center; backdrop-filter:blur(5px);">
        <div style="background:white; width:90%; max-width:500px; padding:30px; border-radius:20px; position:relative;">
...
// LOGO VE NAVBAR GÜNCELLEME (Senin istediğin Puzzle logo)
...
// ÖNERİ GÖNDERME (window._db kullanarak hata payını bitirdik)
async function submitSuggestion() {
    const k = document.getElementById('sugK').value;
    const a = document.getElementById('sugA').value;
    if(!k || !a) return alert("Alanları doldurun.");

    const { error } = await window._db.from('oneriler').insert([{ kelime: k, anlam: a }]);
    if (error) {
        alert("Gönderilemedi, tabloyu veya RLS ayarını kontrol edin.");
    } else {
        alert("🧩 Teşekkürler! Öneriniz alındı.");
        closeModal();
    }
}

// Modal İçerikleri
function showPrivacy() { openModal("<h2>🛡️ Gizlilik</h2><p>Verileriniz güvende.</p>"); }
...
function renderWords(data, title) {
    ...
    if (!data || data.length === 0) { view.innerHTML += `<p>Bulunamadı.</p>`; return; }
    data.forEach(item => {
